...
% ── Metadata ──
\title{Applied NLP}
\subtitle{A Practical Journey Through NLP (Round 1 Revision)}
\author{Roman Jurowetzki}
\institute{Aalborg University / University of Strasbourg}
\date{February 2026 --- Revised Draft}
    ...
    \begin{column}{0.4\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=0.8]
          ...
          \fill[gold, opacity=0.8] (1.3,0.85) rectangle (1.55,1.2);
          \node[font=\tiny, anchor=west, text=gold] at (1.62,1.05) {2 (high IDF)};
...
\begin{frame}{TF-IDF: A Worked Example}
  ...
  \begin{tabular}{lccccp{3.5cm}}
    ...
    oncology & $2/80$ & 8 & 4.83 & \gold{0.121} & very rare, high discrimination \\
    \bottomrule
  \end{tabular}
  \end{center}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{tikzpicture}[scale=0.8]
        ...
        \node[font=\tiny, anchor=west, text=coral] at (7.8,3.35) {Topic A: inflation, rates, GDP};
        \node[font=\tiny, anchor=west, text=gold] at (7.8,2.75) {Topic B: patent, startup, AI};
        \node[font=\tiny, anchor=west, text=blue] at (7.8,2.15) {Topic C: carbon, climate, energy};
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.54\textwidth}
      ...
      \begin{tikzpicture}[scale=0.82]
        ...
        \node[font=\tiny, text=gold, rotate=35] at (3.8,2.75) {large vector distance};
      \end{tikzpicture}
      \end{center}

      \vspace{0.06cm}
      \IfFileExists{figures/generated/ufo.png}{%
        \begin{center}
          \includegraphics[width=0.58\linewidth]{figures/generated/ufo.png}
        \end{center}
      }{%
        \begin{center}
          \fcolorbox{coral}{bgcard}{\parbox{0.85\linewidth}{\centering\tiny Missing image: \texttt{figures/generated/ufo.png}}}
        \end{center}
      }
    \end{column}
  \end{columns}
...
\begin{frame}{The LLM Landscape}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\tiny, text=textmuted] at (3.5,1.3) {Claude Opus 4.7};
...
\begin{frame}{The Model Zoo}
  ...
  \begin{tikzpicture}[scale=0.5]
    \node[font=\tiny, text=textmuted] at (5.8,3.5) {needle-in-a-haystack intuition};
    \foreach \i in {0,...,19} {
      \foreach \j in {0,...,5} {
        \fill[textmuted, opacity=0.28] ({0.5*\i},{0.45*\j}) circle (1.2pt);
      }
    }
...
\begin{frame}{What We'll Build This Week}
  \framesubtitle{Outcomes, systems, and evaluation}
  ...
  \begin{tikzpicture}[
    ...
    \node[dlabel] at (0, 2.5) {Day 1: Foundations \& Scale};
    \node[day] at (0, 0.5) {TF-IDF baselines\\Embedding pipelines\\BERTopic mapping\\Vector similarity\\[0.1cm]\muted{+ first retrieval checks}};

    \node[dlabel] at (5, 2.5) {Day 2: The Retrieval Engine};
    \node[day] at (5, 0.5) {Vector DBs\\FAISS + reranking\\RAG design\\Eval + error analysis\\[0.1cm]\muted{+ retrieval QA loop}};

    \node[dlabel] at (10, 2.5) {Day 3: Production Agents};
    \node[day] at (10, 0.5) {Structured extraction\\Distillation + tuning\\Agentic workflows\\Guardrails + handoff\\[0.1cm]\muted{+ demos + wrap-up}};
  ...
  \vspace{0.3cm}
  \begin{center}
    \muted{Build, compare, and validate: each day turns methods into deployable workflows.}
  \end{center}
\end{frame}
