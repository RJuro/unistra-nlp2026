...
\begin{frame}{TF-IDF Intuition: Signal vs. Noise}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}
        \item Most announcements repeat generic terms: \textbf{assistant}, \textbf{productivity}
        \item Those words are everywhere, so they barely distinguish documents
        \item One announcement includes \gold{clinical-trial evidence}
        \item That rare term is a strong pointer to a specific sub-topic (health/biotech)
      \end{itemize}
      \vspace{0.2cm}
    ...
    \begin{column}{0.4\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=0.8]
          \draw[textmuted, thin] (0,0) rectangle (5.8,4.1);
          \node[font=\scriptsize, text=textdark] at (2.9,4.35) {Document frequency in 100 docs};

          \node[font=\tiny, anchor=west, text=textsecond] at (0.2,3.45) {assistant};
          \fill[textmuted, opacity=0.35] (1.3,3.25) rectangle (5.2,3.6);
          \node[font=\tiny, anchor=west, text=textmuted] at (5.25,3.45) {80};

          \node[font=\tiny, anchor=west, text=textsecond] at (0.2,2.65) {productivity};
          \fill[textmuted, opacity=0.35] (1.3,2.45) rectangle (4.95,2.8);
          \node[font=\tiny, anchor=west, text=textmuted] at (5.0,2.65) {74};

          \node[font=\tiny, anchor=west, text=textsecond] at (0.2,1.85) {copilot};
          \fill[textmuted, opacity=0.35] (1.3,1.65) rectangle (4.7,2.0);
          \node[font=\tiny, anchor=west, text=textmuted] at (4.75,1.85) {68};

          \node[font=\tiny, anchor=west, text=gold] at (0.2,1.05) {clinical-trial};
          \fill[gold, opacity=0.9] (2.05,0.85) rectangle (2.35,1.2);
          \node[font=\tiny, anchor=west, text=gold] at (2.45,1.05) {2 (high IDF)};

          \node[font=\tiny, text=textmuted] at (2.9,0.35) {$\mathrm{IDF}(t)=\log\!\left(\frac{N}{\mathrm{df}(t)}\right)$};
        \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
...
\begin{frame}{Matrix Factorization}
  \framesubtitle{An intuitive map: documents $\rightarrow$ topics $\rightarrow$ words}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{tikzpicture}[scale=0.8]
        % Documents
        \node[font=\footnotesize\bfseries, text=textdark] at (1.4,4.2) {Documents};
        \draw[coral, thick, rounded corners=3pt] (0,1.2) rectangle (2.8,3.9);
        \foreach \y/\lbl in {3.4/Doc 1,2.7/Doc 2,2.0/Doc 3} {
          \draw[textmuted, thin] (0.2,\y-0.35) rectangle (2.6,\y+0.2);
          \node[font=\tiny, anchor=west, text=textsecond] at (0.35,\y-0.07) {\lbl: token counts};
        }

        % Topic mixtures (W)
        \node[font=\footnotesize\bfseries, text=textdark] at (5.2,4.2) {W: topic mixtures};
        \draw[textmuted, thin] (3.4,1.2) rectangle (7.0,3.9);
        % doc bars
        \foreach \y/\a/\b/\c in {3.35/1.5/0.7/0.6,2.7/0.8/1.5/0.8,2.05/0.7/0.8/1.6} {
          \fill[coral, opacity=0.85] (3.6,\y-0.18) rectangle ({3.6+\a},\y+0.18);
          \fill[gold, opacity=0.85] ({3.6+\a},\y-0.18) rectangle ({3.6+\a+\b},\y+0.18);
          \fill[blue, opacity=0.75] ({3.6+\a+\b},\y-0.18) rectangle ({3.6+\a+\b+\c},\y+0.18);
        }
        \node[font=\tiny, text=textmuted] at (5.2,1.45) {each row sums to 1};

        % Topic word distributions (H)
        \node[font=\footnotesize\bfseries, text=textdark] at (9.2,4.2) {H: topic-word dists.};
        \draw[textmuted, thin] (7.6,1.2) rectangle (10.8,3.9);
        \node[font=\tiny, anchor=west, text=coral] at (7.8,3.35) {Topic A: inflation, rates};
        \node[font=\tiny, anchor=west, text=gold] at (7.8,2.75) {Topic B: patent, startup};
        \node[font=\tiny, anchor=west, text=blue] at (7.8,2.15) {Topic C: climate, energy};

        % Arrows
        \draw[-{Stealth}, coral, thick] (2.9,2.55) -- (3.3,2.55);
        \draw[-{Stealth}, coral, thick] (7.1,2.55) -- (7.5,2.55);
      \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.43\textwidth}
      \small
      \textbf{Equivalent views:}
      \begin{itemize}
        \item documents become weighted topic profiles
        \item each topic is a weighted word profile
      \end{itemize}
      \vspace{0.1cm}
      \textbf{Matrix view:}
      \[
      A_{\text{docs}\times\text{words}} \approx
      W_{\text{docs}\times\text{topics}}
      \cdot
      H_{\text{topics}\times\text{words}}
      \]
      \vspace{-0.1cm}
      \begin{itemize}
        \item \textbf{W}: ``how much of each topic in each document''
        \item \textbf{H}: ``which words define each topic''
      \end{itemize}
    \end{column}
  \end{columns}
\nobreak\cmark{Deerwester et al., 1990; Lee \& Seung, 1999}
\end{frame}
...
\begin{frame}[plain]
  \IfFileExists{figures/generated/topic_model_viz.png}{%
    \begin{center}
      \includegraphics[width=0.98\paperwidth,height=0.98\paperheight,keepaspectratio]{figures/generated/topic_model_viz.png}
    \end{center}
  }{%
...
\begin{frame}{Attention Is All You Need}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}\itemsep2pt
        \item RNN-style models are sequential and brittle on long-range dependencies
        \item Attention lets each token directly weigh other tokens
        \item Computation becomes parallel and more context-rich
      \end{itemize}
      \vspace{0.2cm}
      \highlight{Self-attention}: each token scores all others through Q/K/V.\\[0.1cm]
      \small
      \textbf{Query}: what to look for;\enspace
      \textbf{Key}: what each token contains;\enspace
      \textbf{Value}: what it contributes.
    \end{column}
    \begin{column}{0.44\textwidth}
      \highlight{The key equation:}
      \vspace{0.15cm}
      \[
      \text{Attention}(Q,K,V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right) V
      \]
      \vspace{0.1cm}
      \muted{Scale factor $\sqrt{d_k}$ prevents dot products from growing too large.}\\[0.2cm]
      \muted{Multi-head attention runs this in parallel across $h$ heads, each capturing different relationship types.}
    \end{column}
  \end{columns}
...
% --- Slide 23b: Self-Attention Visualization ---
\begin{frame}{Self-Attention: How Tokens Attend to Each Other}
  \framesubtitle{``The cat sat on the mat'' --- which words matter for ``sat''?}

  \begin{center}
  \begin{tikzpicture}[scale=0.95]
    % Token boxes
    \foreach \i/\word in {0/The, 2/cat, 4/sat, 6/on, 8/the, 10/mat} {
      \node[rectangle, draw=textmuted, fill=bgcard, minimum width=1.5cm, minimum height=0.7cm,
            font=\normalsize, text=textdark, line width=0.5pt] (t\i) at (\i, 0) {\word};
    }
    % Highlight "sat" as the query token
    \node[rectangle, draw=coral, fill=bgcard, minimum width=1.5cm, minimum height=0.7cm,
          font=\normalsize\bfseries, text=coral, line width=1.5pt] at (4, 0) {sat};

    % Attention arcs from "sat" to others with varying weights
    \draw[coral, line width=2.5pt, opacity=0.85, bend left=35] (4,0.4) to (2,0.4);
    \node[font=\scriptsize, text=coral] at (3,1.6) {\textbf{0.42}};

    \draw[coral, line width=1.5pt, opacity=0.55, bend left=30] (4,0.4) to (10,0.4);
    \node[font=\scriptsize, text=coral] at (7,1.45) {0.23};

    \draw[coral, line width=0.8pt, opacity=0.35, bend left=30] (4,0.4) to (6,0.4);
    \node[font=\scriptsize, text=textmuted] at (5,0.95) {0.15};

    \draw[coral, line width=0.5pt, opacity=0.2, bend left=35] (4,0.4) to (0,0.4);
    \node[font=\scriptsize, text=textmuted] at (1.5,1.85) {0.08};

    \draw[coral, line width=0.5pt, opacity=0.2, bend left=25] (4,0.4) to (8,0.4);
    \node[font=\scriptsize, text=textmuted] at (6.3,0.85) {0.07};

    \draw[coral, line width=1pt, opacity=0.4, looseness=4, out=120, in=60] (4,0.4) to (4,0.4);
    \node[font=\scriptsize, text=textmuted] at (4,1.4) {0.05};

    % Q/K/V labels below
    \node[rectangle, fill=coral, text=white, font=\scriptsize\bfseries, minimum width=1.5cm, minimum height=0.35cm] at (4,-0.85) {Query (Q)};
    \foreach \i in {0,2,6,8,10} {
      \node[rectangle, fill=gold, text=textdark, font=\tiny\bfseries, minimum width=1.5cm, minimum height=0.3cm] at (\i,-0.85) {Key / Value};
    }
  \end{tikzpicture}
  \end{center}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \small
      \highlight{Reading the diagram:}\\
      Line thickness = attention weight.\\
      ``sat'' attends most to \textbf{``cat''} (the subject)\\
      and \textbf{``mat''} (the location).
    \end{column}
    \begin{column}{0.48\textwidth}
      \small
      ...
      Context is captured \textit{in parallel}.
    \end{column}
  \end{columns}
\end{frame}

% --- Slide 24: The Assembly Line ---
\begin{frame}{The Playlist Curation Analogy}
  \framesubtitle{Each transformer layer adds context}

  \begin{center}
  \begin{tikzpicture}[
    layer/.style={rectangle, draw=coral, fill=bgcard, text=textdark, minimum width=9.6cm, minimum height=0.62cm, font=\footnotesize, line width=0.5pt},
  ]
    \node[layer] (l1) at (0,0) {Layer 1: token-level cues (words and local patterns)};
    \node[layer] (l2) at (0,1.0) {Layer 2: phrase-level semantics (entities, attributes)};
    \node[layer] (l3) at (0,2.0) {Layer 3: relation-level structure (who did what to whom)};
    \node[layer] (l4) at (0,3.0) {Layer 4: discourse-level intent (task-relevant meaning)};
    \draw[-{Stealth}, coral, thick] (0,-0.35) -- (0,-0.08);
    ...
    \node[font=\small, text=gold] at (0,-0.65) {raw tokens};
    \node[font=\small, text=gold] at (0,3.65) {rich contextual representation};
  \end{tikzpicture}
  \end{center}

  \vspace{0.05cm}
  \muted{Like curating a playlist: each pass adds signal until the final ranking reflects deeper context.}
\end{frame}
...
\begin{frame}{The LLM Landscape}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\tiny, text=textmuted] at (3.5,1.3) {Claude Opus 4};
...
\begin{frame}{The Model Zoo}
  ...
  \begin{tabular}{lrlrl}
    ...
    Claude Opus 4 & 200K & Coding & \$3.00 & 77\% SWE-bench \\
...
\begin{frame}{The Cost of Intelligence is Collapsing}
  ...
  \begin{tikzpicture}[scale=0.85]
    \begin{axis}[
      ...
      symbolic x coords={GPT-4 (2023), Claude Sonnet 4, GPT-5.2, DeepSeek R1, GPT-4o Mini, DeepSeek V3.2},
      xtick=data,
      ...
      \addplot[fill=gold, draw=gold] coordinates {(Claude Sonnet 4,15) (GPT-5.2,14) (DeepSeek R1,2.19) (GPT-4o Mini,0.60) (DeepSeek V3.2,0.42)};
    \end{axis}
